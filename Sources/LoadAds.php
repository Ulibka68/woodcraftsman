...
if (file_exists( $sourcedir .'/LoadAdsBanners.php')) {
    require_once($sourcedir . '/LoadAdsBanners.php');
}
...
function loadads()
{
	...
	global $modSettings, $ad_images;
//    $modSettings['ad_images_dir']   - директория для ad-images
...
//    $results = $smcFunc['db_query']('', "SELECT * FROM {db_prefix}ad_images where enabled=1");
    $results = $smcFunc['db_query']('', "SELECT * FROM smf_ad_images where enabled=1 and DATE_FORMAT(date_start, '%Y-%m-%d') <= curdate() and DATE_FORMAT(date_end, '%Y-%m-%d') >= curdate()");
    $ad_images=[];
    while ($row = $smcFunc['db_fetch_assoc']($results))
    {
        $ad_images[] = array(
            'id' => $row['id_ad_images'],
            'name' => $modSettings['ad_images_url'] . "/". $row['filename'],
            'width' => $row['width'],
            'height' => $row['height'],
            'weight' => $row['weight'],
            'url' => $row['url'],
            'free_banner' => $row['free_banner'],
        );
    }
