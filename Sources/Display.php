		...
		if (!empty($modSettings['attachmentEnable']) && allowedTo('view_attachments'))
		{
            $request = $smcFunc['db_query']('', '
                SELECT a.id_attach,
                       ...
                       a.filename,
                       a.file_hash,
                       IFNULL(a.size, 0)          AS filesize,
                       a.downloads,
                       a.approved,
                       a.width,
                       a.height,
                       IFNULL(thumb.id_attach, 0) AS id_thumb,
                       thumb.width                AS thumb_width,
                       thumb.height               AS thumb_height,
                       Imgf.real_fname            AS img_fname,
                       Thumbf.real_fname          AS thumb_fname
                FROM smf_attachments AS a
                         LEFT JOIN smf_attachments AS thumb ON (thumb.id_attach = a.id_thumb)
                         LEFT JOIN smf_attachments_files as Imgf on Imgf.id_attach = a.id_attach
                         LEFT JOIN smf_attachments_files as Thumbf on Thumbf.id_attach = thumb.id_attach
