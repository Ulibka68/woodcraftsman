...
if (file_exists( $sourcedir .'/Mobile_Detect.php')) {
...
function loadUserSettings()
{
	...
	if ($id_member != 0)
	{
		...
		{
			$request = $smcFunc['db_query']('', '
				SELECT mem.*, IFNULL(a.id_attach, 0) AS id_attach, a.filename, a.attachment_type, mts.thanks_count AS thanks
				FROM {db_prefix}members AS mem
					LEFT JOIN {db_prefix}attachments AS a ON (a.id_member = {int:id_member})
					LEFT JOIN {db_prefix}messages_thanks_stats AS mts ON (mts.id_member = {int:id_member})
	...
	$user_info += array(
		...
		'permissions' => array(),
		'thanks' => isset($user_settings['thanks']) ? $user_settings['thanks'] : 0,
	);
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = '
			...
			mem.birthdate, mem.member_ip, mem.member_ip2, mem.icq, mem.aim, mem.yim, mem.msn, mem.posts,
			mts.thanks_count AS thanks, mem.last_login,
...
function detectBrowser()
{
    ...
    // Basic detection. вставил Гайрат
