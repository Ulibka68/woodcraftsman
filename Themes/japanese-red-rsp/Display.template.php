...
//		'print' => array('text' => 'print', 'image' => 'print.gif', 'lang' => true, 'custom' => 'rel="nofollow"', 'url' => $scripturl . '?action=printpage;topic=' . $context['current_topic'] . '.0'),
	...
	echo '
			<div id="forumposts">
				<div class="cat_bar">
					<h3 class="catbg">
						<img src="', $settings['images_url'], '/topic/', $context['class'], '.gif" style="vertical-align: text-bottom;" alt="" />
...
//			Вывести город 
            if (isset($message['member']['location']) && !empty($message['member']['location'])) {
                echo '<li class="thankscount">',$message['member']['location'],'</li>';
            }
		...
		if ($message['can_approve'] || SayThanks::checkCanThank($message) || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'])
			echo '
		...
		if (!empty($message['attachment']))
		{
            ...
            display_attachments2($message['attachment'],$last_approved_state,$message['id']);
...
function display_attachments2($attachments_one_msg_array,$last_approved_state,$message_id) {
    global $context, $settings, $options, $txt, $scripturl, $modSettings,$attachments;

    foreach ($attachments[$message_id] as $attachment)
    {
        echo '<a href="https://woodtoolsimg.ru/frm/', $attachment['img_fname'],
        '" id="link_', $attachment['id_attach'],
        '" onclick="return expandThumb(', $attachment['id_attach'],');',
        '"><img src="https://woodtoolsimg.ru/frm/', $attachment['thumb_fname'],
        '" alt="" id="thumb_', $attachment['id_attach'],
        '" /></a><br />';

        echo '<a href="https://woodtoolsimg.ru/frm/' . $attachment['img_fname'] .
            '"><img src="' . $settings['images_url'] .
            '/icons/clip.gif" align="middle" alt="*" />&nbsp;' .
            $attachment['filename']
            . '</a> ';

        echo '(',
        $attachment['filesize'],
            ', ' . $attachment['width'] . 'x' . $attachment['height'] . ' скачано - '
